...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — python -m pipeline.jd_parser
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print("JDParser — smoke test against hackathon job_description")
    print("=" * 65)

    jd_path = config.JD_PATH
    if not jd_path.exists():
        print(f"[WARN] config.JD_PATH not found at '{jd_path}'.")
        print("       Using built-in sample text for smoke test.")
        # Minimal representative JD text for standalone smoke test
        _SAMPLE_JD = (
            ...
            "Strong Python. Evaluation frameworks: NDCG, MRR, MAP. Hybrid search.\n\n"
            "## Things we'd like you to have but won't reject you for\n"
            "LLM fine-tuning (LoRA, QLoRA, PEFT). Learning-to-rank (XGBoost). MLOps.\n\n"
            "## Things we explicitly do NOT want\n"
            "People who have only worked at consulting firms "
            "(TCS, Infosys, Wipro, Accenture, Cognizant, Capgemini).\n"
            "People whose primary expertise is computer vision, speech, or robotics.\n\n"
            "Experience Required: 5-9 years.\n"
            "Location: Pune/Noida, Hyderabad, Mumbai, Delhi NCR. Relocation accepted.\n"
            "Production experience with deployed systems required.\n"
        )
        jd_source: Union[str, Path] = _SAMPLE_JD
    else:
        jd_source = jd_path
    ...
    try:
        parser = JDParser()
        intent = parser.parse(jd_source, encode=False)  # encode=False: no model needed
    except Exception as exc:
        print(f"[ERROR] JDParser.parse failed: {exc}")
        sys.exit(1)

    print(f"Parser repr : {parser}\n")

    # ── Acceptance criterion 1: required_skills >= 8 ─────────────────────
    print(f"Required skills ({len(intent.required_skills)}):")
    for s in intent.required_skills[:12]:
        print(f"  · {s}")
    if len(intent.required_skills) > 12:
        print(f"  … and {len(intent.required_skills) - 12} more")
    assert len(intent.required_skills) >= _MIN_REQUIRED_SKILLS, (
        f"FAIL: only {len(intent.required_skills)} required skills, "
        f"need >= {_MIN_REQUIRED_SKILLS}"
    )
    print(f"\n[PASS] required_skills >= {_MIN_REQUIRED_SKILLS}  ✓")

    # ── Acceptance criterion 2: consulting disqualifier detected ──────────
    print(f"\ndisqualify_consulting_only : {intent.disqualify_consulting_only}")
    assert intent.disqualify_consulting_only is True, (
        "FAIL: consulting-only disqualifier NOT detected. "
        "Check _detect_consulting_disqualifier logic."
    )
    print("[PASS] disqualify_consulting_only = True  ✓")

    # ── Additional assertions ─────────────────────────────────────────────
    print(f"\nNice-to-have ({len(intent.nice_to_have_skills)}): {intent.nice_to_have_skills[:5]}")
    print(f"Disqualifier skills ({len(intent.disqualifier_skills)}): {intent.disqualifier_skills[:4]}")
    print(f"Expanded required ({len(intent.expanded_required)}): {intent.expanded_required[:6]}")
    print(f"YOE: ideal=[{intent.yoe_ideal_min:.0f}-{intent.yoe_ideal_max:.0f}], "
          f"outer=[{intent.yoe_min:.0f}-{intent.yoe_max:.0f}]")
    print(f"Preferred locations: {intent.preferred_locations}")
    print(f"Relocation accepted: {intent.relocation_accepted}")
    print(f"Production required: {intent.disqualify_no_production}")
    print(f"Embedding          : {intent.embedding}")  # None when encode=False

    assert intent.yoe_ideal_min >= 6.0 and intent.yoe_ideal_max <= 11.0, (
        f"FAIL: YOE ideal range expected [5-9], got "
        f"[{intent.yoe_ideal_min}-{intent.yoe_ideal_max}]"
    )
    print("\n[PASS] YOE ideal range = [5-9]  ✓")

    assert "noida" in intent.preferred_locations or "pune" in intent.preferred_locations, (
        f"FAIL: Noida/Pune not found in locations: {intent.preferred_locations}"
    )
    print("[PASS] Noida/Pune in preferred_locations  ✓")

    assert intent.relocation_accepted is True, "FAIL: relocation_accepted should be True"
    print("[PASS] relocation_accepted = True  ✓")
