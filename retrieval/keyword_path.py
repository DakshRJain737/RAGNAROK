...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — python -m retrieval.keyword_path
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print("KeywordPath — smoke test with synthetic in-memory BM25 index")
    print("=" * 65)

    if not _BM25_AVAILABLE:
        print("[SKIP] rank_bm25 not installed. Cannot run smoke test.")
        sys.exit(0)

    from rank_bm25 import BM25Okapi  # noqa: PLC0415

    # ── Build a synthetic corpus ──────────────────────────────────────────
    # The "sparse" candidate (CAND_0000002) has exact skill keywords but
    # minimal prose — simulates a candidate FAISS would miss.
    CORPUS = [
        # CAND_0000001 — rich profile: embeddings, retrieval, ranking
        "ml engineer embeddings retrieval dense retrieval faiss pinecone "
        "sentence transformers vector search information retrieval ranking "
        "ndcg mrr python pytorch recommendation systems feature engineering".split(),

        # CAND_0000002 — SPARSE profile: only skill names, no prose
        # This is the candidate BM25 should rescue that FAISS would miss.
        "faiss pinecone qdrant elasticsearch opensearch bm25 python".split(),

        # CAND_0000003 — irrelevant: marketing, operations
        "marketing manager operations excel powerpoint project management "
        "customer support tcs infosys consulting sales crm".split(),

        # CAND_0000004 — partial match: some AI keywords
        "data engineer spark airflow sql data pipelines kafka python "
        "machine learning feature engineering scikit-learn".split(),

        # CAND_0000005 — NLP / search engineer background
        "nlp engineer natural language processing hugging face transformers "
        "bert information retrieval text classification python pytorch".split(),
    ]

    CANDIDATE_IDS = [
        "CAND_0000001",  # rich ML profile
        "CAND_0000002",  # sparse — BM25 rescue target
        "CAND_0000003",  # irrelevant
        "CAND_0000004",  # partial match
        "CAND_0000005",  # NLP / search
    ]

    bm25_model = BM25Okapi(CORPUS)
    expander = QueryExpander(config.SKILL_MAP_PATH)
    path_obj = KeywordPath(
    ...
    # ── Build mock JDIntent ───────────────────────────────────────────────
    mock_intent = JDIntent(
        required_skills=[
            "embeddings", "information retrieval", "faiss",
            "pinecone", "python", "ranking", "ndcg",
        ],
        nice_to_have_skills=["lora", "mlops"],
        disqualifier_skills=["computer vision"],
        expanded_required=[],     # not used by KeywordPath
        yoe_min=4.0, yoe_max=12.0,
        yoe_ideal_min=5.0, yoe_ideal_max=9.0,
        preferred_locations=["noida", "pune"],
        relocation_accepted=True,
        disqualify_consulting_only=True,
        disqualify_no_production=True,
        embedding=None,           # not used by KeywordPath
        raw_text="Senior AI Engineer.",
    )

    # ── Run retrieve() ────────────────────────────────────────────────────
    t0 = time.perf_counter()
    results = path_obj.retrieve(mock_intent, top_k=config.KEYWORD_PATH_TOP_K)
    elapsed_ms = (time.perf_counter() - t0) * 1000.0

    print(f"\nRetrieved {len(results)} results in {elapsed_ms:.2f} ms\n")
    for r in results:
        marker = "  ← sparse rescue" if r.candidate_id == "CAND_0000002" else ""
        print(
            f"  rank={r.rank_in_path}  {r.candidate_id}  "
            f"score={r.path_score:.4f}{marker}"
        )

    result_ids = [r.candidate_id for r in results]

    # ── Acceptance criterion 1: sparse candidate rescued ──────────────────
    assert "CAND_0000002" in result_ids, (
        "FAIL: Sparse candidate CAND_0000002 not retrieved by BM25. "
        "It has exact keyword matches (faiss, pinecone, qdrant) that should "
        "score high regardless of missing prose."
    )
    print("\n[PASS] Sparse-profile candidate CAND_0000002 rescued  ✓")

    # ── Acceptance criterion 2: irrelevant candidate NOT in top-3 ─────────
    if "CAND_0000003" in result_ids:
        irrel_rank = result_ids.index("CAND_0000003") + 1
        assert irrel_rank > 3, (
            f"FAIL: Irrelevant marketing candidate ranked {irrel_rank} "
            "— expected rank > 3."
        )
    print("[PASS] Irrelevant candidate (marketing/ops) not in top-3  ✓")
    ...
    print("[PASS] Results sorted by BM25 score descending  ✓")

    # ── Acceptance criterion 4: scores in [0, 1] ──────────────────────────
    assert all(0.0 <= r.path_score <= 1.0 for r in results), (
        "FAIL: path_score out of [0, 1] range"
    )
    print("[PASS] All path_scores in [0.0, 1.0]  ✓")

    # ── Acceptance criterion 5: all path_name == "keyword" ────────────────
    assert all(r.path_name == KeywordPath.PATH_NAME for r in results)
    print("[PASS] All results have path_name='keyword'  ✓")
    ...
    # ── Acceptance criterion 7: error on empty required_skills ────────────
    empty_intent = JDIntent(
        required_skills=[], nice_to_have_skills=[], disqualifier_skills=[],
        expanded_required=[], yoe_min=0, yoe_max=50,
        yoe_ideal_min=5, yoe_ideal_max=9,
        preferred_locations=[], relocation_accepted=False,
        disqualify_consulting_only=False, disqualify_no_production=False,
        embedding=None, raw_text="",
    )
    empty_results = path_obj.retrieve(empty_intent)
    assert empty_results == [], (
        "FAIL: Expected [] for empty required_skills, "
        f"got {len(empty_results)} results."
    )
    print("[PASS] Empty required_skills returns []  ✓")

    print(f"\n[INFO] Query tokens sample: "
          f"{path_obj._build_query_tokens(mock_intent)[:10]}")
