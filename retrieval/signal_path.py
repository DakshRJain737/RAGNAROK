...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — python -m retrieval.signal_path
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print("SignalPath — smoke test: highly-engaged vs inactive candidates")
    print("=" * 65)

    # ── Synthetic feature matrix ──────────────────────────────────────────
    # Columns:  recency  resp_rate  open  notice  github  complete  interview
    DATA = np.array([
        # CAND_0000031 — highly engaged: active, open, low notice, responsive
        [0.95,  0.91,  1.0,   1.00,  0.33,  0.83,  0.60],
        # CAND_0000043 — active, not open, medium notice
        [0.82,  0.40,  0.0,   0.75,  0.37,  0.57,  0.72],
        # CAND_0000002 — inactive 8 months, low response, 90-day notice
        [0.10,  0.29,  1.0,   0.10,  0.50,  0.79,  0.62],
        # CAND_0000014 — recent, high response, not open, 90-day notice
        [0.88,  0.80,  0.0,   0.00,  0.50,  0.62,  0.55],
        # CAND_0000038 — very active, open, great signals
        [0.98,  0.20,  1.0,   1.00,  0.38,  0.36,  0.75],
        # CAND_0000003 — barely active, zero github, long notice
        [0.08,  0.46,  0.0,   0.00,  0.50,  0.32,  0.86],
        # CAND_0000015 — open, decent signals
        [0.75,  0.32,  1.0,   0.50,  0.50,  0.79,  0.72],
        # CAND_0000048 — good github, recently joined, open
        [0.90,  0.65,  1.0,   0.00,  0.00,  0.62,  0.42],
    ], dtype=np.float32)

    IDS = np.array([
        "CAND_0000031",  # target: highly engaged
        "CAND_0000043",
        "CAND_0000002",  # target: inactive — should score low
        "CAND_0000014",
        "CAND_0000038",
        "CAND_0000003",
        "CAND_0000015",
    ...
    path = SignalPath(feature_data=DATA, candidate_ids=IDS)
    print(f"\nBuilt: {path}\n")
    print(f"Weight vector: {dict(zip(_WEIGHT_KEYS, path._weights.tolist()))}\n")

    # ── Run retrieve() ────────────────────────────────────────────────────
    t0 = time.perf_counter()
    results = path.retrieve(top_k=config.SIGNAL_PATH_TOP_K)
    elapsed_ms = (time.perf_counter() - t0) * 1000.0
    ...
    for r in results:
        ...
        if r.candidate_id == "CAND_0000031":
            tag = "  ← highly engaged (target)"
        elif r.candidate_id == "CAND_0000002":
            tag = "  ← inactive (should rank low)"
        print(
            f"  rank={r.rank_in_path}  {r.candidate_id}"
            f"  score={r.path_score:.4f}{tag}"
        )

    # ── Acceptance criterion 1: highly-engaged candidate in results ───────
    assert "CAND_0000031" in result_ids, (
        f"FAIL: Highly-engaged CAND_0000031 not retrieved. "
        f"Results: {result_ids}"
    )
    print("\n[PASS] Highly-engaged candidate CAND_0000031 retrieved  ✓")

    # ── Acceptance criterion 2: engaged candidate outranks inactive ───────
    assert "CAND_0000002" in result_ids, "CAND_0000002 missing from results"
    rank_engaged  = result_ids.index("CAND_0000031") + 1
    rank_inactive = result_ids.index("CAND_0000002") + 1
    assert rank_engaged < rank_inactive, (
        f"FAIL: engaged rank={rank_engaged} should be < "
        f"inactive rank={rank_inactive}"
    )
    print(
        f"[PASS] Engaged (rank {rank_engaged}) outranks "
        f"inactive (rank {rank_inactive})  ✓"
    )
    ...
    # ── Acceptance criterion 5: path_name = "signal" ─────────────────────
    assert all(r.path_name == SignalPath.PATH_NAME for r in results)
    print("[PASS] All results have path_name='signal'  ✓")
    ...
    # ── Acceptance criterion 7: manual score verification ────────────────
    # CAND_0000031 row: [0.95, 0.91, 1.0, 1.00, 0.33, 0.83, 0.60]
    # weights:          [0.25, 0.20, 0.15, 0.15, 0.10, 0.10, 0.05]
    expected_31 = (
        0.25 * 0.95 + 0.20 * 0.91 + 0.15 * 1.0 + 0.15 * 1.00
        + 0.10 * 0.33 + 0.10 * 0.83 + 0.05 * 0.60
    )
    actual_31 = next(r.path_score for r in results if r.candidate_id == "CAND_0000031")
    assert abs(actual_31 - expected_31) < 1e-4, (
        f"FAIL: expected score {expected_31:.4f}, got {actual_31:.4f}"
    )
    print(f"[PASS] Manual score check: CAND_0000031 = {actual_31:.4f}"
          f" (expected {expected_31:.4f})  ✓")

    # ── Acceptance criterion 8: explain_candidate breakdown ──────────────
    idx_31 = list(IDS).index("CAND_0000031")
    breakdown = path.explain_candidate(idx_31)
    assert set(breakdown.keys()) == set(_WEIGHT_KEYS)
    assert abs(sum(breakdown.values()) - actual_31) < 1e-4
    print(f"[PASS] explain_candidate breakdown sums to score  ✓")
    print(f"       {breakdown}")

    # ── Acceptance criterion 9: weight vector sums to 1.0 ────────────────
    assert abs(float(path._weights.sum()) - 1.0) < 1e-5
    print("[PASS] Weight vector sums to 1.0  ✓")

    # ── Acceptance criterion 10: n_candidates property ───────────────────
