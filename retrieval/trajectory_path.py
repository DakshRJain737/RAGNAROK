...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — python -m retrieval.trajectory_path
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print("TrajectoryPath — smoke test: IC-riser at product company")
    print("=" * 65)

    # ── Synthetic trajectory data ─────────────────────────────────────────
    #         promo/yr  yoe  prod_co  ic_riser  consult_only
    DATA = np.array([
        [0.75,  6.0, 1.0, 1.0, 0.0],  # CAND_0000031 IC-riser, product co, ideal YOE
        [0.50,  7.5, 1.0, 0.0, 0.0],  # CAND_0000043 solid, product co
        [0.00,  8.0, 0.0, 0.0, 1.0],  # CAND_0000002 stagnant, consulting only
        [1.20,  9.0, 1.0, 1.0, 0.0],  # CAND_0000014 fast-riser, product co, top of band
        [0.25,  2.5, 1.0, 0.0, 0.0],  # CAND_0000011 junior, product co
        [0.30, 15.0, 0.0, 0.0, 1.0],  # CAND_0000050 over-experienced, consulting
        [0.40,  5.5, 1.0, 0.0, 0.0],  # CAND_0000038 mid, product co, in band
        [0.00,  0.5, 0.0, 0.0, 0.0],  # CAND_0000003 fresher, no history
    ], dtype=np.float32)

    IDS = np.array([
        "CAND_0000031",   # target: IC-riser, product co
        "CAND_0000043",
        "CAND_0000002",   # should score low: consulting only
        "CAND_0000014",
        "CAND_0000011",   # should score low: junior
        "CAND_0000050",   # should score low: over-experienced + consulting
        "CAND_0000038",
        "CAND_0000003",   # should score near-zero: fresher
    ], dtype=object)
    ...
    # ── Build mock JDIntent ───────────────────────────────────────────────
    mock_jd = JDIntent(
        required_skills=["embeddings", "information retrieval", "python"],
        nice_to_have_skills=["mlops"],
        disqualifier_skills=["computer vision"],
        expanded_required=[],
        yoe_min=config.YOE_BAND_MIN,
        yoe_max=config.YOE_BAND_MAX,
        yoe_ideal_min=config.YOE_BAND_IDEAL_MIN,
        yoe_ideal_max=config.YOE_BAND_IDEAL_MAX,
    ...
    # ── Run retrieve() ────────────────────────────────────────────────────
    t0 = time.perf_counter()
    results = path.retrieve(mock_jd, top_k=config.TRAJECTORY_PATH_TOP_K)
    elapsed_ms = (time.perf_counter() - t0) * 1000.0
    ...
    for r in results:
        row_idx = list(IDS).index(r.candidate_id)
        row = DATA[row_idx]
        tag = ""
        if r.candidate_id == "CAND_0000031":
            tag = "  ← target: IC-riser + product co"
        elif r.candidate_id == "CAND_0000002":
            tag = "  ← consulting only (penalised)"
        print(
            f"  rank={r.rank_in_path}  {r.candidate_id}"
            f"  score={r.path_score:.4f}"
            f"  (promo={row[COL_PROMOTIONS_PER_YEAR]:.2f}"
            f"  yoe={row[COL_YOE]:.1f}"
            f"  prod={int(row[COL_HAS_PRODUCT_CO])}"
            f"  ic={int(row[COL_IS_IC_RISER])}"
            f"  cons={int(row[COL_CONSULTING_ONLY])})"
            f"{tag}"
        )

    # ── Acceptance criterion 1: IC-riser at product co in results ─────────
    assert "CAND_0000031" in result_ids, (
        f"FAIL: IC-riser + product-co candidate CAND_0000031 not in results. "
        f"Got: {result_ids}"
    )
    print("\n[PASS] IC-riser at product company in results  ✓")

    # ── Acceptance criterion 2: IC-riser outranks consulting-only ─────────
    if "CAND_0000002" in result_ids:
        ic_rank   = result_ids.index("CAND_0000031") + 1
        cons_rank = result_ids.index("CAND_0000002") + 1
        assert ic_rank < cons_rank, (
            f"FAIL: IC-riser rank={ic_rank} should be < "
            f"consulting-only rank={cons_rank}"
        )
    print("[PASS] IC-riser outranks consulting-only candidate  ✓")
    ...
    for i in range(len(results) - 1):
        assert results[i].path_score >= results[i + 1].path_score, (
            ...
            f"{results[i].path_score:.4f} < {results[i+1].path_score:.4f}"
        )
    print("[PASS] Scores sorted descending  ✓")

    # ── Acceptance criterion 4: scores in (0, 1] ─────────────────────────
    assert all(0.0 < r.path_score <= 1.0 for r in results), (
        f"FAIL: score out of (0, 1]: {[r.path_score for r in results]}"
    )
    print("[PASS] All path_scores in (0.0, 1.0]  ✓")

    # ── Acceptance criterion 5: path_name = "trajectory" ─────────────────
    assert all(r.path_name == TrajectoryPath.PATH_NAME for r in results)
    print("[PASS] All results have path_name='trajectory'  ✓")

    # ── Acceptance criterion 6: rank_in_path sequential from 1 ───────────
    assert [r.rank_in_path for r in results] == list(range(1, len(results) + 1))
    print("[PASS] rank_in_path is 1-indexed sequential  ✓")

    # ── Acceptance criterion 7: candidate outside YOE band scores 0 ──────
    # CAND_0000050: yoe=15.0 > soft_max=12.0 → should be excluded
    assert "CAND_0000050" not in result_ids, (
        f"FAIL: over-experienced candidate CAND_0000050 (yoe=15) "
        f"appeared in results despite being outside soft YOE bound."
    )
    print("[PASS] Candidate outside YOE soft bound excluded  ✓")

    # ── Acceptance criterion 8: YOE band scoring correctness ─────────────
    yoe_vals = np.array([3.0, 5.0, 7.0, 9.0, 11.0, 13.0], dtype=np.float32)
    yoe_scores = _yoe_band_score(yoe_vals, 5.0, 9.0, 4.0, 12.0)
    assert float(yoe_scores[1]) == 1.0, "YOE=5.0 should score 1.0"
    assert float(yoe_scores[2]) == 1.0, "YOE=7.0 should score 1.0"
    assert float(yoe_scores[3]) == 1.0, "YOE=9.0 should score 1.0"
    assert float(yoe_scores[0]) < 1.0,  "YOE=3.0 should score < 1.0 (below soft_min=4)"
    assert float(yoe_scores[4]) < 1.0,  "YOE=11.0 should score < 1.0 (above ideal_max)"
    assert float(yoe_scores[5]) == 0.0, "YOE=13.0 should score 0.0 (above soft_max=12)"
    print("[PASS] _yoe_band_score returns correct values  ✓")

    # ── Acceptance criterion 9: n_candidates property ────────────────────
    assert path.n_candidates == len(IDS)
    print(f"[PASS] n_candidates = {path.n_candidates}  ✓")
