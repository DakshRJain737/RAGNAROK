...
# --------------------------------------------------------------------------- #
# Quick smoke test — run as: python -m ontology.query_expander               #
# --------------------------------------------------------------------------- #
...
if __name__ == "__main__":
    ...
    test_cases: list[tuple[str, list[str]]] = [
        # Sprint acceptance criterion 1: dense retrieval → 5+ terms
        ("dense retrieval → synonyms only",
         expander.expand_skills(["dense retrieval"], include_co_skills=False)),

        # Sprint acceptance criterion 2: full expansion with co-skills
        ("dense retrieval → with co-skills",
         expander.expand_skills(["dense retrieval"], include_co_skills=True)),

        # JD core skills
        ("JD required core",
         expander.expand_skills([
             "embeddings", "information retrieval",
             "sentence transformers", "faiss"
         ])),

        # Reverse domain transfer: information retrieval ← recommendation systems
        ("information retrieval ← domain sources",
         expander.get_domain_transfer_sources("information retrieval")),

        # BM25 tokens output
        ("BM25 tokens for 'ranking'",
         expander.build_query_tokens(["ranking", "ndcg"])),
    ]
    ...
    # Assert sprint acceptance criterion: dense retrieval expands to >= 5 terms
    dense_synonyms = expander.expand_skills(
        ["dense retrieval"], include_co_skills=False
    )
    assert len(dense_synonyms) >= 5, (
        f"FAIL: 'dense retrieval' expanded to only {len(dense_synonyms)} terms; "
        f"expected >= 5. Terms: {dense_synonyms}"
    )
    print("[PASS] 'dense retrieval' → synonyms-only expansion = "
          f"{len(dense_synonyms)} terms >= 5 ✓")

    # Assert RecSys transfers to IR
    ir_sources = expander.get_domain_transfer_sources("information retrieval")
    assert "recommendation systems" in ir_sources, (
        f"FAIL: 'recommendation systems' not in IR reverse-transfer sources. "
        f"Got: {ir_sources}"
    )
    print("[PASS] 'recommendation systems' is a reverse-transfer source "
