...
Sprint acceptance criteria (Phase 3, DEV A, Day 11):
  ✓ Weights sum to 1.0
      Config-level tests confirm WEIGHT_SKILL + WEIGHT_CAREER + WEIGHT_BEHAVIORAL = 1.0
      and sum(BEHAVIORAL_WEIGHTS.values()) = 1.0. These pass NOW regardless of
      composite.py status.
  ✓ Non-increasing enforced
      Tested in TestComposite (marked PENDING — requires composite.py from DEV B).
  ✓ Consulting penalty correct
      Tested in TestCareerQuality (marked PENDING — requires career_quality.py from DEV B).
  ✓ All edge cases covered
      Fully covered for behavioral.py, trajectory.py, honeypot_filter.py.
      Edge cases for skill_match/career_quality/composite are specified in PENDING stubs.

Coverage by file:
  scoring/behavioral.py     COMPLETE  ← full tests in TestBehavioralScorer
  scoring/trajectory.py     COMPLETE  ← full tests in TestTrajectoryVelocityScorer
  scoring/honeypot_filter.py STUB     ← tests in TestHoneypotFilter
  scoring/cross_encoder.py   MINE     ← fallback + sigmoid tests in TestCrossEncoderFallback
  scoring/career_quality.py  EMPTY    ← PENDING (TestCareerQuality — skip marked)
  scoring/skill_match.py     EMPTY    ← PENDING (TestSkillMatch — skip marked)
  scoring/composite.py       EMPTY    ← PENDING (TestComposite — skip marked)

Running:
    pytest tests/test_scoring.py -v
    pytest tests/test_scoring.py -v -k "not PENDING"     # only ready tests
    pytest tests/test_scoring.py -v -k "Config"          # config-level only
    pytest tests/test_scoring.py -v -k "Behavioral"      # behavioral only
    pytest tests/test_scoring.py -v -k "Trajectory"      # trajectory only

DEV B action items (empty files):
    Once career_quality.py / skill_match.py / composite.py are implemented,
    remove the @pytest.mark.skip decorator from the corresponding test class.
    Each pending class includes full interface specification as comments so
    DEV B knows exactly what the tests expect.

Dependencies:
    config.py                 weight constants, thresholds
    pipeline/schemas.py       CareerEntry (trajectory), RRFResult (cross_encoder)
    scoring/behavioral.py     BehavioralScorer, BehavioralResult
    scoring/trajectory.py     TrajectoryVelocityScorer, count_promotions,
                              trajectory_velocity_score, TrajectoryResult
    scoring/honeypot_filter.py HoneypotCleanup
    scoring/cross_encoder.py  CrossEncoderReranker, _sigmoid (via import)
    tests/conftest.py         mock_jd_intent, app_config
"""
